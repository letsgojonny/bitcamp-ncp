...
public class MemberDaoImpl implements MemberDao {
  ...
  public void insert(Member m) {
    try (Statement stmt = con.createStatement()) {
      ...
      String sql = String.format(
          "insert into app_member(name, email, pwd, tel) values('%s', '%s', sha2('%s', 256), '%s')",
          m.getName(),
          m.getEmail(),
          m.getPassword(),
          m.getTel());
    ...
    try (Statement stmt = con.createStatement();
        ResultSet rs = stmt.executeQuery(
            "select member_id, name, email, tel, created_date"
                + " from app_member where member_id="+no)) {
  ...
  public int update(Member m) {
    try (Statement stmt = con.createStatement()) {
      ...
      String sql = String.format("update app_member set name='%s', email='%s', pwd=sha2('%s', 256), "
          + "tel='%s' where member_id=%d",
          m.getName(), m.getEmail(), m.getPassword(), m.getTel(), m.getNo());
